...
using MangaRack.Provider.Batoto.Extension;
using TinyHttp;
        ...
        private HtmlDocument _Artists {
            set {
                Artists = value.DocumentNode.Descendants("td")
                    ...
                    .SelectMany(x => x.ParentNode.LastElement().Descendants("a").Select(y => HtmlEntity.DeEntitize(y.InnerText).Trim()))
                    .ToArray();
            }
        }
        ...
        private HtmlDocument _Genres {
            set {
                Genres = value.DocumentNode.Descendants("td")
                    ...
                    .SelectMany(x => x.ParentNode.LastElement().Descendants("span").Select(y => HtmlEntity.DeEntitize(y.InnerText).Trim()))
                    .ToArray();
            }
        }
        ...
        private HtmlDocument _Summary {
            set {
                Summary = string.Join("\n", value.DocumentNode.Descendants("td")
                    ...
                    .SelectMany(x => x.ParentNode.LastElement().Descendants().Select(y => HtmlEntity.DeEntitize(y.InnerText).Trim()))
                    .ToArray())
                    .Replace("\n\n", "\n");
            }
        }
