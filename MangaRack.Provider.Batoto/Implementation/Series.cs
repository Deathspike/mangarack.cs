			...
			get {
				...
				if (!_Contains(() => Chapters)) {
					...
					Listing[] Listings = _HtmlDocument.DocumentNode.Descendants("a")
						...
						.Select(x => new { Chapter = x, Match = Regex.Match(HtmlEntity.DeEntitize(x.InnerText).Trim(), @"(\s?Vol\.\s?(?<Volume>[0-9\.]+))?\s?(Ch\.)?\s?(?<Number>([0-9\.]+[a-z]?|Extra|Omake))(\s?-\s?[0-9\.]+)?(\s?v\.?[0-9]+)?(\s?\(?Part\s(?<Part>[0-9]+)\)?)?(\s?(-|\+)\s?)?\s?(Read Onl?ine|:?\s?(?<Title>.+?)?(Read Online)?)$", RegexOptions.IgnoreCase) })
						// ... where the previous match was successful ...
						.Where(x => x.Match.Success)
						// ... selecting a proper type with all relevant information ...
						.Select(x => new Listing(x.Match.Groups["Number"].Value.AlphabeticToNumeric()) { Chapter = x.Chapter, Match = x.Match, Volume = double.TryParse(x.Match.Groups["Volume"].Value, out ProcessedVolume) ? ProcessedVolume : -1, Title = x.Match.Groups["Title"].Value })
