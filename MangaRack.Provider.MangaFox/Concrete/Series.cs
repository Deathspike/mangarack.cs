	...
	sealed class Series : ISeries {
		...
		private HtmlDocument _Artists {
			set {
				...
				Artists = value.DocumentNode.Descendants("a")
					...
					.Where(x => HtmlEntity.DeEntitize(x.GetAttributeValue("href", string.Empty)).StartsWith("/search/artist/"))
					// ... select the text ...
		...
		private HtmlDocument _Authors {
			set {
				...
				Authors = value.DocumentNode.Descendants("a")
					// ... with a reference starting with the appropriate address ...
					.Where(x => HtmlEntity.DeEntitize(x.GetAttributeValue("href", string.Empty)).StartsWith("/search/author/"))
					// ... select the text ...
					.Select(x => HtmlEntity.DeEntitize(x.InnerText))
					// ... and create a list.
					.ToList();
		...
		private HtmlDocument _Chapters {
			set {
				...
				Chapters = value.DocumentNode.Descendants("h3")
					...
					.Where(x => HtmlEntity.DeEntitize(x.GetAttributeValue("class", string.Empty)).Split(' ').Contains("volume"))
					// ... select each valid volume ...
					.Select(x => new { Match = Regex.Match(HtmlEntity.DeEntitize(x.FirstChild.InnerText), @"^Volume\s(?<Volume>.+)$", RegexOptions.IgnoreCase), ChapterListing = x.ParentNode.NextSibling })
					...
					.SelectMany(x => x.ChapterListing.Descendants("a")
						...
						.Where(y => HtmlEntity.DeEntitize(y.GetAttributeValue("href", string.Empty)).Contains("/manga/"))
						// ... and select each chapter with a number ...
						.Select(y => new Chapter(double.TryParse(Regex.Match(HtmlEntity.DeEntitize(y.InnerText), "(?<Number>[0-9.]+)$", RegexOptions.IgnoreCase).Groups["Number"].Value, out ProcessedNumber) ? ProcessedNumber : -1,
							// ... with a title ...
							y.ParentNode.Descendants("span")
								// ... with a class indicating a title ...
								.Where(z => HtmlEntity.DeEntitize(y.GetAttributeValue("class", string.Empty)).Split(' ').Contains("title"))
								// ... select the text ...
								.Select(z => HtmlEntity.DeEntitize(y.InnerText))
		...
		private HtmlDocument _Genres {
			set {
				...
				Genres = value.DocumentNode.Descendants("a")
					// ... with a reference starting with the appropriate address ...
					.Where(x => HtmlEntity.DeEntitize(x.GetAttributeValue("href", string.Empty)).StartsWith("?/search/genres/"))
					// ... select the text ...
					.Select(x => HtmlEntity.DeEntitize(x.InnerText))
					// ... and create a list.
					.ToList();
		...
		private HtmlDocument _Summary {
			set {
				...
				Summary = string.Join("\n", value.DocumentNode.Descendants("p")
					// ... with a class indicating a summary ...
					.Where(x => HtmlEntity.DeEntitize(x.GetAttributeValue("class", string.Empty)).Split(' ').Contains("summary"))
					// ... select each split summary piece ...
					.SelectMany(x => Regex.Split(HtmlEntity.DeEntitize(x.InnerText), "\r\n"))
		...
		private HtmlDocument _Title {
			set {
				...
				Title = value.DocumentNode.Descendants("title")
					...
					.Select(x => new { Match = Regex.Match(HtmlEntity.DeEntitize(x.InnerText), @"^(?<Title>.+)(\s+)Manga(\s+)-", RegexOptions.IgnoreCase) })
