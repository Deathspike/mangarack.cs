			...
			set {
				...
				Children = value.DocumentNode.Descendants("h3")
					...
					.SelectMany(x => x.ChapterListing.Descendants("a")
						...
						.Select(y => new Chapter(double.TryParse(Regex.Match(HtmlEntity.DeEntitize(y.InnerText).Trim(), "(?<Number>[0-9.]+)$", RegexOptions.IgnoreCase).Groups["Number"].Value, out ProcessedNumber) ? (double?)ProcessedNumber : null,
							// ... with a location ...
							...
							y.ParentNode.ParentNode.Descendants("a")
								...
								.FirstOrDefault(),
							// ... with a volume.
							double.TryParse(x.Match.Groups["Volume"].Value, out ProcessedVolume) ? (double?)ProcessedVolume : null) as IChapter))
					// ... reverse the order ...
					.Reverse()
					// ... and create an array.
					.ToArray();
