		...
		public void Populate(Action<IChapter> Done) {
			...
			Http.Get(UniqueIdentifier.EndsWith("1.html") ? UniqueIdentifier : UniqueIdentifier + "1.html", (Response) => {
				...
				Pages = HtmlDocument.DocumentNode.Descendants("select")
					...
					.Where(x => HtmlEntity.DeEntitize(x.GetAttributeValue("onchange", string.Empty)).StartsWith("change_page"))
					// ... select each option element ...
					.Select(x => x.Descendants("option"))
					// ... select the first set of elements ...
					.First()
					// ... with a possible value ...
					.Where(x => !HtmlEntity.DeEntitize(x.GetAttributeValue("value", string.Empty)).Equals("0"))
					// ... select each page ...
					.Select(x => new Page(string.Join("/", UniqueIdentifier.Split('/').TakeWhile(y => !y.EndsWith(".html")).ToArray()) + "/" + HtmlEntity.DeEntitize(x.GetAttributeValue("value", string.Empty)) + ".html") as IPage)
					// ... and create a list.
					.ToList();
				// Invoke the callback.
				Done(this);
