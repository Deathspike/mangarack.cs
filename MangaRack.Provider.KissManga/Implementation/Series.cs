			...
			get {
				...
				if (!_Contains(() => Chapters)) {
					...
					_Set(() => Chapters, _HtmlDocument.DocumentNode.Descendants("a")
						...
						.Select(x => new { Chapter = x, Match = Regex.Match(HtmlEntity.DeEntitize(x.InnerText).ReplaceWhile(".0", "."), @"(\s?Vol\.\s?(?<Volume>[0-9]+))?\s?(Ch\.\s?)?(?<Number>[0-9.]+):?\s?(?<Title>.+?)?$", RegexOptions.IgnoreCase) })
