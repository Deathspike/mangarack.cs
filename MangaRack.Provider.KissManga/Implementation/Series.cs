			...
			get {
				...
				if (!_Contains(() => Chapters)) {
					...
					_Set(() => Chapters, _HtmlDocument.DocumentNode.Descendants("a")
						...
						.Select(x => new { Chapter = x, Match = Regex.Match(HtmlEntity.DeEntitize(x.InnerText).ReplaceWhileWithDigit(".0", ".").Trim(), @"(\s?Vol\.\s?(?<Volume>[0-9]+))?\s?(Ch\.)?\s?(?<Number>[0-9\.]+)(-[0-9\.]+)?(\s?v\.?[0-9]+)?(\s?\(?Part\s(?<Part>[0-9]+)\)?)?(\s?(-|\+)\s?)?\s?(Read Onl?ine|:?\s?(?<Title>.+?)?(Read Online)?)$", RegexOptions.IgnoreCase) })
